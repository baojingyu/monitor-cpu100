...
awk_command="awk -v cpu_usage=\"$cpu_usage\" -v threshold=\"$threshold\" 'BEGIN { if (cpu_usage >= threshold) exit 1; else exit 0; }'"
...
echo "awk result: $awk_result"
  ...
  if awk -v cpu_usage="$cpu_usage" -v threshold="$threshold" 'BEGIN { if (cpu_usage >= threshold) exit 1; else exit 0; }'; then
    ...
    if awk -v cpu_usage="$cpu_usage" -v threshold="$threshold_message_push" 'BEGIN {if (cpu_usage >= threshold) exit 0; else exit 1}'; then
      # 转义特殊符号
